...
hline!(plt6, [1.0], color=:red, linestyle=:dash, label="Порог эпидемии (R_e=1)", linewidth=1.5)

# Отметка момента, когда R_e становится < 1
...
if !isnothing(cross_idx) && cross_idx > 1
    ...
    annotate!(plt6, cross_time, 1.2,
        text("R_e<1 с $(round(cross_time, digits=1)) дня", 8, :left))
end
