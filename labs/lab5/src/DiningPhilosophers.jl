...
function plot_marking_evolution(df::DataFrame, N::Int)
    # Создаём 4 графика
    p_think = plot(xlabel="Time", ylabel="Count", title="Think states")
    p_hungry = plot(xlabel="Time", ylabel="Count", title="Hungry states")
    p_eat = plot(xlabel="Time", ylabel="Count", title="Eat states")
    p_fork = plot(xlabel="Time", ylabel="Count", title="Fork states")
    
    for i = 1:N
        # Think
        col_name = "Think_$i"
        if col_name in names(df)
            plot!(p_think, df.time, df[!, col_name], label="Think_$i")
        end
        
        # Hungry
        col_name = "Hungry_$i"
        if col_name in names(df)
            plot!(p_hungry, df.time, df[!, col_name], label="Hungry_$i")
        end
        
        # Eat
        col_name = "Eat_$i"
        if col_name in names(df)
            plot!(p_eat, df.time, df[!, col_name], label="Eat_$i")
        end
        
        # Fork
        col_name = "Fork_$i"
        if col_name in names(df)
            plot!(p_fork, df.time, df[!, col_name], label="Fork_$i")
        end
    end
    
    # Объединяем графики
    return plot(p_think, p_hungry, p_eat, p_fork, layout=(4,1), size=(800,1000))
end
