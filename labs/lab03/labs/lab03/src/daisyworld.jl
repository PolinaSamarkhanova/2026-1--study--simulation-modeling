...
function update_surface_temperature!(pos, model)
    ...
    local_heating = absorbed_luminosity > 0.5 ? 72 * log(absorbed_luminosity) + 80 : 80
...
function daisyworld(;
    ...
    model = StandardABM(Daisy, space; properties, agent_step! = daisy_step!, model_step! = daisyworld_step!, rng=rng)
    ...
    for wp in white_positions
        add_agent!(wp, model, :white, rand(rng, 1:max_age), albedo_white)
    end
    ...
    for bp in black_positions
        add_agent!(bp, model, :black, rand(rng, 1:max_age), albedo_black)
    end
