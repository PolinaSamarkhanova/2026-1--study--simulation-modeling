...
function initialize_sir(;
    ...
    B_und = [0.5, 0.5, 0.5],         # передача невыявленными
    B_det = [0.05, 0.05, 0.05],      # передача выявленными
    infection_period = 14,
    detection_time = 7,
    death_rate = 0.02,
    reinfection_probability = 0.1,
    Is = [5, 5, 1],                  # начальное количество инфицированных
    seed = 42,
    n_steps = 100,
)
    rng = Xoshiro(seed)
...
function transmit!(agent, model)
    ...
    rate = if agent.days_infected < model.detection_time
        model.B_und[agent.pos]   # невыявленный — высокая заразность
    else
        model.B_det[agent.pos]   # выявленный — низкая заразность
    end
    ...
    # Получаем всех агентов в том же локации, кроме самого заразного
